/* ==========================================================================
   data.js
   --------------------------------------------------------------------------
   This file holds ALL the scholarship/scheme data used across the whole
   website. Every page (home, results, schemes, scheme details) reads from
   this same file so we never have the same scholarship typed out twice.

   IMPORTANT — DATA DISCLAIMER
   This is an ILLUSTRATIVE / DEMO dataset built for a college project.
   Scheme names are inspired by real Indian scholarship programs, but the
   exact amounts, ceilings and dates below should NOT be treated as
   officially verified. Anyone using this site for real should always
   check the official portal linked on each scheme's detail page.
   ========================================================================== */

// The list of qualification levels used everywhere in the app.
// Both the eligibility form dropdown AND the scheme data below use these
// exact string values, so they always match up correctly.
...
// Social category options used in the eligibility form and filters.
...
// States covered by this demo. Keep this list consistent with the "state"
// field used inside SCHOLARSHIPS below.
const STATE_OPTIONS = ["Punjab", "Haryana"];

/**
 * SCHOLARSHIPS — the main dataset.
 *
 * Each scheme object looks like this:
 * {
 *   id:            unique text ID, used in the URL (scheme.html?id=...)
 *   name:          scheme name
 *   provider:      the ministry / department / organisation running it
 *   level:         "central" or "state"
 *   state:         state name if level is "state", otherwise null
 *   minAge/maxAge: the eligible age range (inclusive)
 *   qualifications: array of qualification values (see QUALIFICATION_LEVELS)
 *   categories:    array of category values. ["any"] means everyone qualifies
 *   incomeCeiling: text describing the family income limit, if any
 *   amount:        the benefit amount, shown as the headline figure
 *   about:         a short paragraph describing the scheme
 *   portal:        official (or clearly-marked demo) URL for the scheme
 *   verifiedOn:    the date this demo entry was last checked/updated
 *   notes:         any extra eligibility notes worth mentioning
 * }
 */
const SCHOLARSHIPS = [
  {
